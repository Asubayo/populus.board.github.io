...
(function (factory) {
    ...
    Mapael.prototype = {
        ...
        init: function () {
			...
			// Hook that allows to configure the paper before drawing anything (background)
			if (self.options.map.beforeDrawMapAreas) self.options.map.beforeDrawMapAreas(self.$container, self.paper, self.options);
        ...
        initDelegatedMapEvents: function() {
            ...
            self.$container.on("click." + pluginName, "[data-id]", function (evt, opts) {
                ...
                var type = $elem.attr('data-type');
                if (dataTypeToElementMapping[type] !== undefined) {
                    self.elemClick(dataTypeToElementMapping[type][id], id);
                } else if (type === 'legend-elem' || type === 'legend-label') {
                    var legendIndex = $elem.attr('data-legend-id');
                    var legendType = $elem.attr('data-legend-type');
            ...
            $.each(self.customEventHandlers, function(eventName) {
                ...
                self.$container.off(fullEventName).on(fullEventName, "[data-id]", function (e) {
                        ...
                        self.customEventHandlers[eventName][type][id] !== undefined)
                    {
						...
						console.log(elem.options.eventHandlers);
        ...
        onUpdateEvent: function (e, opt) {
			...
			if(opt.paperOptions)
			{
				...
				/*self.paper.image(
					...
					0, 
					0, 
					opt.paperOptions.imageWidth, 
					opt.paperOptions.imageHeight);
					console.log("self.options.map.width : "+self.options.map.width);
				if (self.options.map.width) {
        ...
        elemClick: function (elem, id) {
			...
			if (elem.options.onClick) {
				elem.options.onClick(elem, id);
